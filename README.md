...
| 配置项         | 命令行参数      | 环境变量      | 示例值                           |
| -------------- | --------------- | ------------- | -------------------------------- |
| **目标 API URL**   | `--target-url`    | `TARGET_URL`    | `https://your-target-webui-url.com` |
| **监听端口**       | `--port`          | `LISTEN_PORT`   | `8080`                             |

#### Bash/Linux/macOS 示例
```bash
# 替换 'YOUR_TARGET_URL' 和 'YOUR_PORT' 为实际值
export TARGET_URL="https://your-target-webui-url.com"
...
```powershell
$env:TARGET_URL = "https://your-target-webui-url.com"
...
#### 直接命令行参数运行
```bash
python proxy.py --target-url https://your-target-webui-url.com --port 8080
```
...
## 📜 免责声明
**本项目仅供学习和技术研究使用，严禁用于任何商业或非法目的。**

用户在使用本项目的过程中，必须严格遵守目标网站/服务的使用条款和相关法律法规。通过使用此代理，您承认并同意：
- 您对自己的行为负全部责任。
- 因使用本项目而导致的任何法律纠纷、数据泄露、账户封禁或经济损失，项目作者概不负责。
- 本项目不提供任何形式的保证，不对其可用性、安全性或准确性作任何承诺。

请在合法合规的前提下使用本工具。如果您不同意以上条款，请勿使用本项目。
```